...
begin
  if not InternalStrToDate(DateFormat, S, Result) then
    raise EConvertError.CreateFmt({$IFDEF Delphi3} SInvalidDate {$ELSE}
...
initialization
{!touched!}{$IfDef LogInit} WriteLn('W:\common\components\rtl\Garant\L3\l3Date.pas initialization enter'); {$EndIf}
  {initialize DefaultYear and DefaultMonth}
  SetDefaultYear;
  // Всегда четыре цифры в годе (иначе все отъезжает на NT)
  FourDigitYear := True; {Pos('YYYY', AnsiUpperCase(ShortDateFormat)) > 0;}

{!touched!}{$IfDef LogInit} WriteLn('W:\common\components\rtl\Garant\L3\l3Date.pas initialization leave'); {$EndIf}
end.
