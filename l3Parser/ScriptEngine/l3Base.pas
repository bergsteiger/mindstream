...
{ $Id: l3Base.pas,v 1.579 2015/08/28 13:52:09 lulin Exp $ }
...
procedure Tl3ThreadContainer.pm_SetSuspended(Value: Bool);
  {-}
begin
 if (f_Suspended <> Value) then begin
  f_Suspended := Value;
  if (ToolThread = nil) then begin
   if not Value AND not Terminated then begin
    f_Executed := False;
    if NeedThread then begin
     f_ToolThread := f_ToolThreadClass.Create(true);
     with ToolThread do begin
